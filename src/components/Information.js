...
const Information = () => {
  ...
  return (
    <div id="cuandoydonde" className="w-full order-6 ">
      <div className="text-center xl:text-left text-3xl pt-8 mt-10 mb-2 lg:ml-48 font-alex-brush text-color-camel">
        Preguntas
      </div>
      <div className="text-center xl:text-left text-3xl lg:ml-48 mb-10 font-bold">
        CUÁNDO & DONDE
      </div>

      <div className="w-full flex flex-col md:flex-row md:justify-center">
        {organization.map((v, k) => (
          <SubInformation key={k} model={v} />
        ))}
      </div>
    </div>
  );
